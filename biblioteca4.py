...
def ajusta_acervo(tituloLivro):
    ...
    for i in range(0, len(lista_livros)):
        if (lista_livros[i]['titulo']) == tituloLivro:
            ...
            atualiza_qtd = input("          »» Deseja atualizar a quantidade disponivel para locação [S/N]? ")
            if atualiza_qtd in "Ss":
                lista_livros[i]["qtdisponivel"] = lista_livros[i]["qtdisponivel"] + (qtd_nova - qtd_atual)
                print('\n   :: Quantidade de exemplares disponíveis para locação atualizada com sucesso! ::')
            else:
                print('\n   :: Quantidade de exemplares disponíveis para locação inalterada! ::')
            break
